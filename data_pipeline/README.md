...
Run `python pipeline.py` after `pip install -r requirements.txt`. It scrapes the first five catalogue pages, giving 100 books, cleans values and computes INR at the fixed assignment rate 105.50 per GBP. [query_results.md](query_results.md) records all query strings, outputs, and the SQL/pandas join check. Parse failures in numeric values use median imputation; missing identity/category/availability is dropped because it cannot be reliably reconstructed.
